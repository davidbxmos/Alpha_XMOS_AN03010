...
// Flag to make sure a given model is initialised only once. This is needed because the model/trained_model_xcore.cpp file have one time initialised non-const global
...
int32_t vnr_init(vnr_model_quant_spec_t * quant_spec) {
    ...
    {
        model_initialised = 1;
        int32_t ret = model_init((void*)quant_spec->c_flash);
